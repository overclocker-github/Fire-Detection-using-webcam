...
fire_cascade = cv2.CascadeClassifier('C:\\Users\\yadav\\Documents\\Program in Python\\fire_detection_cascade_model.xml') # To access xml file which includes positive and negative images of fire. (Trained images)
...
def play_alarm_sound_function(): # defined function to play alarm post fire detection using threading
    playsound.playsound('C:\\Users\\yadav\\Documents\\Program in Python\\fire_alarm.mp3',True) # to play alarm # mp3 audio file is also provided with the code.
...
def send_mail_function(): # defined function to send mail post fire detection using threading
    ...
    recipientmail = "yadavansiaditya@gmail.com" # recipients mail
    ...
    try:
        ...
        server.ehlo()
        server.starttls()
        server.login("yadavansiaditya@gmail.com", 'tesrect123') # Senders mail ID and password
        server.sendmail('yadavansiaditya@mail.com', recipientmail, "Warning fire accident has been reported") # recipients mail with mail message
